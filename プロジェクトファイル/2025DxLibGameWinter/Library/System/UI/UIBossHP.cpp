...
namespace {
	constexpr float kScale = 0.5f * Statistics::kScreenWidthFullHDRatio;

	const Position2 kDrawPos = Position2(
		Statistics::kScreenWidth * 0.75f,
		Statistics::kScreenHeight * 0.8f);
	const Vector2 kGaugeOffset = Vector2(15, -62) * kScale;

	// アニメーションを行う時間
	constexpr int kShakeAnimTime = 10;
	constexpr int kDecAnimTime = 120;
	// アニメーションを行い始める時間
	constexpr int kStartDecAnimTime = kDecAnimTime - 80;
	// 振動の大きさ(カメラの振動量依存)
...
{
	float currentHp = _boss.lock()->GetHitPoint();
...
void UIBossHP::Draw(Vector2 shakeOffset)
{
	float hp = _boss.lock()->GetHitPoint();
	float ratio = 0.0f;
	// HPがあった場合
	if (hp > 0.0f) {
		ratio = hp / _maxHp;
...
void UIBossHP::UpdateAnim()
{
	// アニメーションの更新
	UpdateAnimDec();
}

void UIBossHP::UpdateAnimDec()
{
...
void UIBossHP::DrawHPBase(Position2 drawPos) const
{
	int w, h;
...
void UIBossHP::DrawHPGauge(Position2 drawPos, float ratio) const
{
...
void UIBossHP::DrawHPGaugeDec(Position2 drawPos, float ratio)
{
