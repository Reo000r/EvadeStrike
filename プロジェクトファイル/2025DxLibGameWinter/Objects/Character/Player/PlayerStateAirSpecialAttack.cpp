...
PlayerStateAirSpecialAttack::PlayerStateAirSpecialAttack(std::weak_ptr<Player> player, 
	...
	PlayerStateAirAttack(player, data, attackCol, PlayerActionKind::Special, rawActionKind)
{
}

void PlayerStateAirSpecialAttack::OnEnter()
{
	// 攻撃判定行列をモデルの前にする
...
void PlayerStateAirSpecialAttack::Update()
{
	// ステートが変わっていた場合return
...
void PlayerStateAirSpecialAttack::OnExit()
{
	// 攻撃判定を消す
	_attackCol->Disable();
	// 重力影響度を元に戻す
