...
PlayerStateAirDodge::PlayerStateAirDodge(std::weak_ptr<Player> player) :
	PlayerStateBase(player)
{
}

void PlayerStateAirDodge::OnEnter()
{
	// 方向入力があった場合は
	if (CanDashInput()) {
		// 入力方向の反対に向ける
...
void PlayerStateAirDodge::Update()
{
	// ステートが変わっていた場合return
...
void PlayerStateAirDodge::OnExit()
{
	// ジャスト回避判定を消す
	_justDodgeCollider->Disable();
	// 重力影響度を元に戻す
...
std::shared_ptr<PlayerStateBase> PlayerStateAirDodge::CheckStateTransition()
{
	// 入力が効く状態でないかつ
...
bool PlayerStateAirDodge::CanJustDodgeColActive(float currentFrameRate)
{
	return (currentFrameRate > kStartJustDodgeFrameRate &&
		currentFrameRate < kEndJustDodgeFrameRate);
}

void PlayerStateAirDodge::StartJustDodge()
{
	// ジャスト回避通知
